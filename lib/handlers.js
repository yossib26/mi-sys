...
async function duplicateCampaignGift(pool, campaignId, giftId, brandScope, actor) {
  await assertCampaignAccess(pool, campaignId, brandScope);
  const original = await pool.query(
    'SELECT sku, name, image, image_mime, product_id FROM campaign_gifts WHERE id = $1 AND campaign_id = $2',
    [giftId, campaignId]
  );
  ...
  const result = await pool.query(
    `INSERT INTO campaign_gifts (campaign_id, sku, name, image, image_mime, position, product_id)
     VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING ${CAMPAIGN_GIFT_COLUMNS}`,
    [campaignId, o.sku, newName, o.image, o.image_mime, position, o.product_id]
  );
