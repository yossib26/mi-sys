...
const CAMPAIGN_COLUMNS = `
  ...
  c.views_count, c.form_starts_count,
...
async function assertCampaignAccess(pool, campaignId, brandScope) {
  ...
  if (!brandScope.includes(result.rows[0].brand_id)) {
    throw httpError(403, 'you do not have access to this brand');
  }
  ...
  if (result.rows[0].status === 'archived') {
    throw httpError(403, 'archived campaigns are admin-only');
  }
}
...
async function duplicateCampaign(pool, id, brandScope) {
  ...
  const original = await pool.query(
    `SELECT brand_id, name, description, budget, start_date, end_date, template, banner, banner_mime
     FROM campaigns WHERE id = $1`,
    [id]
  );
  if (original.rowCount === 0) throw httpError(404, 'campaign not found');
  const o = original.rows[0];
  const newName = `${o.name} (עותק)`;

  const inserted = await pool.query(
    `INSERT INTO campaigns (brand_id, name, description, budget, start_date, end_date, status, template, banner, banner_mime)
     VALUES ($1, $2, $3, $4, $5, $6, 'draft', $7, $8, $9) RETURNING id`,
    [o.brand_id, newName, o.description, o.budget, o.start_date, o.end_date, o.template, o.banner, o.banner_mime]
  );
...
async function updateCampaign(pool, id, body, brandScope) {
  await assertCampaignAccess(pool, id, brandScope);
...
async function getCampaignBySlug(pool, brandSlug, campaignSlug) {
  ...
  if (campaign.status !== 'active') throw httpError(404, 'campaign not found');
...
async function setCampaignBanner(pool, id, body, brandScope) {
  await assertCampaignAccess(pool, id, brandScope);
...
async function deleteCampaignBanner(pool, id, brandScope) {
  await assertCampaignAccess(pool, id, brandScope);
  const result = await pool.query(
