...
`;
const NETWORK_COLUMNS = `id, name, (logo IS NOT NULL) AS has_logo, active, created_at`;
...
// Active-only by default, which is what every consumer other than the
// catalog page itself wants (the campaign editor's picker). The
// management page passes include_inactive so it can still show and
// reactivate a retired network.
async function listNetworks(pool, query) {
  const includeInactive = !!(query && (query.include_inactive === '1' || query.include_inactive === 'true'));
  const result = await pool.query(
    `SELECT ${NETWORK_COLUMNS} FROM networks ${includeInactive ? '' : 'WHERE active'} ORDER BY name`
  );
  return { status: 200, body: result.rows };
}

// The logo is optional and can be supplied right here at creation, as
// the same data URI setNetworkLogo takes — saving the "create, then
// upload" round trip the table row still offers for changing it later.
async function createNetwork(pool, body) {
  const name = ((body && body.name) || '').trim();
  if (!name) throw httpError(400, 'שם הוא שדה חובה');
  const active = body && body.active !== undefined ? !!body.active : true;
  // Parsed before the insert, so a rejected image fails the whole
  // creation rather than leaving a logo-less network behind.
  const logo = body && body.image
    ? parseFileDataUri(body.image, ALLOWED_LOGO_MIME_TYPES, MAX_LOGO_BYTES)
    : null;
  try {
    const result = await pool.query(
      `INSERT INTO networks (name, active, logo, logo_mime) VALUES ($1, $2, $3, $4) RETURNING ${NETWORK_COLUMNS}`,
      [name, active, logo ? logo.buffer : null, logo ? logo.mime : null]
    );
    return { status: 201, body: result.rows[0] };
  } catch (error) {
...
// `active` alone may be sent (the status toggle in the table), in
// which case the name is left as it is — otherwise flipping a status
// would demand resending a name the toggle never asked for.
async function updateNetwork(pool, id, body) {
  const hasName = body && body.name !== undefined;
  const hasActive = body && body.active !== undefined;
  if (!hasName && !hasActive) throw httpError(400, 'לא סופקו שדות לעדכון');

  const name = hasName ? String(body.name).trim() : null;
  if (hasName && !name) throw httpError(400, 'שם הוא שדה חובה');

  const sets = [];
  const values = [];
  if (hasName) { values.push(name); sets.push(`name = $${values.length}`); }
  if (hasActive) { values.push(!!body.active); sets.push(`active = $${values.length}`); }
  values.push(id);

  try {
    const result = await pool.query(
      `UPDATE networks SET ${sets.join(', ')} WHERE id = $${values.length} RETURNING ${NETWORK_COLUMNS}`,
      values
    );
    if (result.rowCount === 0) throw httpError(404, 'הרשת לא נמצאה');
    return { status: 200, body: result.rows[0] };
...
// Inactive networks are filtered out here too, not just from the
// catalog picker: a retired network shouldn't appear anywhere in the
// campaign editor. Its campaign_networks row survives regardless (see
// setCampaignNetworks), so reactivating it brings it back to this list.
async function listCampaignNetworks(pool, campaignId, brandScope) {
  ...
  const result = await pool.query(
    `SELECT n.id, n.name, (n.logo IS NOT NULL) AS has_logo FROM campaign_networks cn JOIN networks n ON n.id = cn.network_id
     WHERE cn.campaign_id = $1 AND n.active ORDER BY cn.position, n.name`,
    [campaignId]
  );
  return { status: 200, body: result.rows };
}
...
// Only the *active* selections are replaced. The editor can't see
// inactive ones (listCampaignNetworks hides them), so they're absent
// from every set it sends — deleting them here would silently discard
// a campaign's selection the moment anyone saved the section after a
// network was retired, and reactivating it later wouldn't bring it
// back. Their rows are left exactly as they are instead.
async function setCampaignNetworks(pool, campaignId, networkIds, brandScope, actor) {
  ...
  await pool.query(
    `DELETE FROM campaign_networks cn USING networks n
     WHERE cn.network_id = n.id AND cn.campaign_id = $1 AND n.active`,
    [campaignId]
  );
  if (ids.length > 0) {
    try {
      ...
      // ON CONFLICT rather than a plain insert: an inactive network's
      // row survived the delete above, so a caller that submits its id
      // anyway should just reposition it, not hit a duplicate-key error.
      await pool.query(
        `INSERT INTO campaign_networks (campaign_id, network_id, position) VALUES ${values}
         ON CONFLICT (campaign_id, network_id) DO UPDATE SET position = EXCLUDED.position`,
        params
      );
    } catch (error) {
...
async function createRegistration(pool, campaignId, body) {
  ...
  // Counts only what the public page actually rendered: a campaign
  // whose every selected network has since been retired shows no
  // network section at all, so it must not still demand a pick.
  const networksCountResult = networksEnabled
    ? await pool.query(
      `SELECT COUNT(*) AS n FROM campaign_networks cn JOIN networks n ON n.id = cn.network_id
       WHERE cn.campaign_id = $1 AND n.active`,
      [campaignId]
    )
    : { rows: [{ n: 0 }] };
  ...
  if (Number(networksCountResult.rows[0].n) > 0) {
    ...
    if (networkId) {
      const networkResult = await pool.query(
        `SELECT n.id, n.name FROM campaign_networks cn JOIN networks n ON n.id = cn.network_id
         WHERE cn.campaign_id = $1 AND cn.network_id = $2 AND n.active`,
