...
async function describeCampaignFieldChange(pool, field, value) {
  ...
  if (field === 'activetrail_sync_enabled' || field === 'networks_enabled') return `${label} ${value ? 'הופעל' : 'כובה'}`;
...
async function reserveCampaignGift(pool, campaignId, giftId) {
  const giftCheck = await pool.query('SELECT id FROM campaign_gifts WHERE id = $1 AND campaign_id = $2', [giftId, campaignId]);
...
async function createRegistration(pool, campaignId, body) {
  ...
  const campaignResult = await pool.query('SELECT gift_mode, networks_enabled FROM campaigns WHERE id = $1', [campaignId]);
  const giftMode = campaignResult.rows[0] && campaignResult.rows[0].gift_mode;
  const networksEnabled = campaignResult.rows[0] && campaignResult.rows[0].networks_enabled;
