...
const UPDATABLE_CAMPAIGN_FIELDS = [
  ...
  'activetrail_token', 'activetrail_group_id', 'activetrail_email_field_id', 'gift_mode', 'slug',
];
...
const CAMPAIGN_COLUMNS = `
  ...
  c.views_count, c.form_starts_count, c.gift_mode,
...
const CAMPAIGN_GIFT_COLUMNS = `
  id, campaign_id, sku, name, (image IS NOT NULL) AS has_image, position, product_id, created_at
`;
...
// every campaign (see ensureMarketingConsentField) rather than added
// freely — only its label is editable. Sorting fixed fields last (not
// by their `position`) is how "always last, regardless of what else
// gets added/reordered" is enforced everywhere fields are listed.
...
async function listCampaignActivityLog(pool, campaignId, brandScope) {
  await assertCampaignAccess(pool, campaignId, brandScope);
  const result = await pool.query(
    `SELECT id, user_id, username, action, created_at FROM campaign_activity_log
...
const CAMPAIGN_LOG_FIELD_LABELS = {
  ...
  activetrail_token: 'טוקן ActiveTrail', activetrail_group_id: 'קוד קבוצת ActiveTrail',
  activetrail_email_field_id: 'שדה אימייל ל-ActiveTrail',
};
...
async function listBrands(pool, brandScope) {
  if (brandScope) {
    const result = await pool.query('SELECT * FROM brands WHERE id = ANY($1) ORDER BY name', [brandScope]);
    return { status: 200, body: result.rows };
  }
...
async function createCampaign(pool, body, brandScope) {
  ...
  try {
    ...
    await ensureMarketingConsentField(pool, campaign.id);
    return {
      status: 201,
...
async function duplicateCampaign(pool, id, brandScope) {
  ...
  const original = await pool.query(
    `SELECT brand_id, name, description, budget, start_date, end_date, template, banner, banner_mime, gift_mode
  ...
  const inserted = await pool.query(
    `INSERT INTO campaigns (brand_id, name, description, budget, start_date, end_date, status, template, banner, banner_mime, gift_mode)
     VALUES ($1, $2, $3, $4, $5, $6, 'draft', $7, $8, $9, $10) RETURNING id`,
    [o.brand_id, newName, o.description, o.budget, o.start_date, o.end_date, o.template, o.banner, o.banner_mime, o.gift_mode]
  );
  ...
  // Safety net in case the original predates this field (shouldn't
  // happen once every campaign is backfilled, but costs nothing).
  await ensureMarketingConsentField(pool, newId);
...
// Provisions the built-in marketing-consent checkbox for a campaign if
// it doesn't already have one — called on campaign creation/duplication.
// ON CONFLICT DO NOTHING makes it safe to call speculatively (e.g. as a
// duplicate's safety net) without an extra existence check.
async function ensureMarketingConsentField(pool, campaignId) {
  await pool.query(
    `INSERT INTO campaign_fields (campaign_id, type, label, required, options, position, fixed_key)
     VALUES ($1, 'checkbox', $2, false, NULL, 0, $3)
...
// A fixed field (see ensureMarketingConsentField) only allows editing
...
async function createCampaignGift(pool, campaignId, body, brandScope, actor) {
  await assertCampaignAccess(pool, campaignId, brandScope);
  const { sku, name } = parseGiftInput(body);
  ...
  try {
    const result = await pool.query(
      `INSERT INTO campaign_gifts (campaign_id, sku, name, position, product_id)
       VALUES ($1, $2, $3, $4, $5) RETURNING ${CAMPAIGN_GIFT_COLUMNS}`,
      [campaignId, sku, name, position, productId]
    );
...
async function updateCampaignGift(pool, campaignId, giftId, body, brandScope, actor) {
  ...
  const { sku, name } = parseGiftInput(body);
  const result = await pool.query(
    `UPDATE campaign_gifts SET sku = $1, name = $2 WHERE id = $3 AND campaign_id = $4 RETURNING ${CAMPAIGN_GIFT_COLUMNS}`,
    [sku, name, giftId, campaignId]
  );
...
// — see ensureMarketingConsentField). A campaign missing any of the
// three settings, or a registrant who left consent unchecked, has
// sync skipped — this just returns quietly rather than erroring.
// Every other submitted value (bar the email field itself, the
// consent checkbox, and any file uploads, which aren't meaningful as
// flat text) rides along as ActiveTrail customFields, keyed by the
// field's own label.
//
...
async function maybeSyncRegistrationToActiveTrail(pool, campaignId, fields, customFields) {
  try {
    const { rows } = await pool.query(
      'SELECT activetrail_token, activetrail_group_id, activetrail_email_field_id FROM campaigns WHERE id = $1',
      [campaignId]
    );
    const campaign = rows[0];
    if (!campaign || !campaign.activetrail_token || !campaign.activetrail_group_id || !campaign.activetrail_email_field_id) return;
...
async function listRegistrations(pool, campaignId, brandScope) {
  await assertCampaignAccess(pool, campaignId, brandScope);
  const result = await pool.query(
