...
async function login(pool, body) {
  ...
  if (!username || !password) throw httpError(400, 'שם משתמש וסיסמה הם שדות חובה');
  ...
  if (!user || !(await verifyPassword(password, user.password_hash))) {
    throw httpError(401, 'שם משתמש או סיסמה שגויים');
  }
...
async function updateUser(pool, id, body) {
  ...
  if (target.rowCount === 0) throw httpError(404, 'המשתמש לא נמצא');

  if (body && body.password) {
    if (body.password.length < 4) throw httpError(400, 'הסיסמה חייבת להכיל לפחות 4 תווים');
  ...
  if (Array.isArray(body && body.brand_ids)) {
    try {
      ...
      if (error.code === '23503') throw httpError(400, 'אחד או יותר מהמותגים שנבחרו אינו קיים');
      throw error;
    }
  }
...
async function createUser(pool, body) {
  ...
  if (!username || !password) throw httpError(400, 'שם משתמש וסיסמה הם שדות חובה');
  if (password.length < 4) throw httpError(400, 'הסיסמה חייבת להכיל לפחות 4 תווים');
  if (!ROLES.includes(role)) throw httpError(400, `תפקיד חייב להיות אחד מ: ${ROLES.join(', ')}`);
  ...
  try {
    ...
    if (error.code === '23505') throw httpError(409, 'שם המשתמש כבר קיים במערכת');
    throw error;
  }
...
async function deleteUser(pool, id, currentUserId) {
  if (String(id) === String(currentUserId)) {
    throw httpError(400, 'לא ניתן למחוק את החשבון שלך עצמך');
  }
  ...
  if (target.rows[0].role === 'admin') {
    ...
    if (parseInt(adminCount.rows[0].n, 10) <= 1) {
      throw httpError(400, 'לא ניתן למחוק את המנהל האחרון שנותר');
    }
  }
