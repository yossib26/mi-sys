...
// server-wide: one SMS account for the whole system. This file doesn't
// go looking for them — it's handed a config object
// ({ apiUrl, user, password, sender }) that getSmsConfig in
// lib/handlers.js resolves from the admin settings page, falling back
// to the UNICELL_API_URL / UNICELL_USER / UNICELL_PASSWORD /
// UNICELL_SENDER environment variables.
//
// ---------------------------------------------------------------
...
// an API URL is configured, isConfigured() returns false and the OTP
// handlers refuse to start a challenge with a clear error rather than
// pretending a code was sent.
// ---------------------------------------------------------------

function isConfigured(config) {
  return !!(config && config.apiUrl && config.user && config.password);
}
...
// about the format.
function buildRequest(config, phone, text) {
  return {
    url: config.apiUrl,
    options: {
      method: 'POST',
...
// was never sent.
async function sendSms(config, phone, text) {
  if (!isConfigured(config)) {
    throw Object.assign(new Error('שירות ה-SMS אינו מוגדר. יש להשלים את ההגדרות בעמוד "הגדרות SMS".'), { statusCode: 503 });
  }
  const { url, options } = buildRequest(config, normalizePhone(phone), text);
