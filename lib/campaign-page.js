...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Same 60px contain-fit as the footer logo, so a wordmark and a
     square icon both sit correctly without being distorted. Centred
     above the title: the dialog covers the page, and the brand is what
     tells the visitor whose code they're about to be sent. */
  .otp-brand-logo { display: block; width: 60px; height: 60px; object-fit: contain; border-radius: 8px; margin: 0 auto 14px; }
...
<div class="otp-overlay" id="otpOverlay" hidden role="dialog" aria-modal="true" aria-labelledby="otpTitle">
  <div class="otp-dialog">
    ${campaign.brand_has_logo ? `<img class="otp-brand-logo" src="/api/brands/${campaign.brand_id}/logo" alt="${escapeHtml(campaign.brand_name)}" />` : ''}
    <h3 id="otpTitle">אימות מספר טלפון</h3>
    <p class="otp-sent-to" id="otpSentTo"></p>
    <!-- placeholder/maxlength start at the 6 digits the server
         currently issues and are re-synced from the send response's
         code_length, so they follow OTP_CODE_DIGITS if it ever moves. -->
    <input class="otp-code-input" id="otpCode" type="text" inputmode="numeric" autocomplete="one-time-code"
      maxlength="6" pattern="\\d{6}" placeholder="XXXXXX" aria-label="קוד האימות" />
...
<script>
  ...
  // The number is always the one already answered in the form's phone
  // field — the dialog never offers to change it. That keeps the two
  // in step by construction: the server only accepts a registration
  // whose phone answer is the number the challenge was verified
  // against, so there is nothing here that could make them disagree.
  async function runOtpChallenge(phone) {
    ...
    let codeLength = codeInput.maxLength || 6;
    ...
    async function requestCode() {
      ...
      if (data.code_length) {
        ...
        // Same "shape of the value" convention as the form's own phone
        // and date fields — a template, not a sample code.
        codeInput.placeholder = 'X'.repeat(codeLength);
      }
    ...
    codeInput.value = '';
    setMsg('');
    sentToEl.textContent = 'שלחנו קוד בן ' + codeLength + ' ספרות ל-' + phone;
  ...
  // Test mode: /c/:brand/:campaign?otp_test=1&phone=05X-XXXXXXX opens
  // the dialog as soon as the page loads, so the SMS step can be
  // exercised without filling in the whole form first. Deliberately
  // behind a query parameter and not on by default — a live campaign
  // page must not greet ordinary visitors with a verification dialog
  // before they've typed anything.
  //
  // The dialog always texts whatever is in the form's phone field, so
  // the phone parameter fills that field first — the number still
  // comes from the form, exactly as it does in the real flow, it's
  // just been seeded from the URL. Without it there'd be nothing to
  // send to on a freshly loaded page.
  //
  // On success this only reports back: no registration is submitted,
  // and the challenge is left unspent.
  if (OTP_ENABLED && new URLSearchParams(location.search).has('otp_test')) {
    window.addEventListener('DOMContentLoaded', async () => {
      ...
      if (formPhone && params.get('phone')) formPhone.value = params.get('phone');

      const phone = formPhone ? formPhone.value.trim() : '';
      if (!phone) {
        msgEl.textContent = 'מצב בדיקה: יש להוסיף מספר לכתובת, למשל ?otp_test=1&phone=0501234567';
        msgEl.className = 'form-msg error';
        return;
      }
      const result = await runOtpChallenge(phone);
      if (result && result.error) {
        msgEl.textContent = 'מצב בדיקה: ' + result.error;
        msgEl.className = 'form-msg error';
      } else if (result && result.challengeId) {
        msgEl.textContent = 'מצב בדיקה: המספר ' + phone + ' אומת בהצלחה (לא נשמרה הרשמה).';
