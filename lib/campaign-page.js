...
<script>
  ...
  // pagehide covers normal navigation/close/back-forward-cache more
  // reliably than beforeunload across browsers, so both are wired for
  // maximum coverage (releaseGift's own fetch is idempotent-ish: an
  // already-released or nonexistent gift is just a harmless no-op
  // server-side).
  window.addEventListener('pagehide', () => releaseGift(selectedGiftId));
  window.addEventListener('beforeunload', () => releaseGift(selectedGiftId));
