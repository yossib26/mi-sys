...
// Combines brand + campaign name into one slug, with the numeric id
// appended to guarantee uniqueness regardless of how the names
// transliterate.
function buildSlug(brandName, campaignName, id) {
  const parts = [slugify(brandName), slugify(campaignName), String(id)].filter(Boolean);
  return parts.join('-');
}

module.exports = { slugify, transliterate, buildSlug };
